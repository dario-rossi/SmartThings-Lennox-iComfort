...
*
*  Version History:  
*  2.1.6a -started with code from tslagle13 - Modified to enable/disable UV based on Sky1/2.  Added 24hr Eain, reformatted wind and rain data with line breaks and fixed mm rain calculations
...
def getTempColors() {
	def colorMap = []
    if (state?.tempMetric) {
		log.info("Colour Map is using Celsius")
        colorMap = [
			// Celsius Color Range
		...
		log.info("Colour Map is using Fahrenheit")
        colorMap = [
...
def updated() {
    ...
    if (!("true" == enableStorm)) {
        ...
        sendEvent(name:"windDirection", value: "O", displayed:false)
...
private def callAPI() {
    ...
    try {
        ...
        httpGet([
            ...
            if (state.debug) log.debug "Sky Raw Data is: " + data
            if (data) {
                ...
                //first find out if device type is Sky1 so that we can filter out the Lux
                def deviceTypeKey = data.find{ it.key == "DeviceType" }
                def deviceTypeSky = deviceTypeKey.value
                def deviceTypeSky1 = false
                if (deviceTypeSky == "SKY1") {
                		deviceTypeSky1 == true
                    } else {
                    	deviceTypeSky1 == false
					}
				...
				data.each {oldkey, datum->
                    ...
                    if (state.debug) log.debug "Sky1/2 Data: Key:Data is ${key}:${datum}"
                    ...
                    switch(key) {
                        ...
                        case "luminance": //update illuminance
                            if ("true" == deviceTypeSky1) {
                            		sendEvent(name: "illuminance", value: datum, unit: "Lux")
                                }
                             else {
                                	sendEvent(name: "illuminance", value: "Lux n/a on Sky2", unit: "")
                                }
                        break;
                        case "uvindex": //update uvindex
                            if ("true" == deviceTypeSky1) {
                            	sendEvent(name: "ultravioletIndex", value: datum, unit: "UV")
							}
							else if ("false" == deviceTypeSky1 && ("false" == enableStorm) ) {
								sendEvent(name: "ultravioletIndex", value: "UV Index n/a on Sky2", unit: "")
							}
                        break;
                        case "pressure": 
			...
			if (state.debug && ("true" == enableStorm)) log.debug "Storm Raw Data is: " + data
            if (data && ("true" == enableStorm)) {
                if (state.debug) log.debug "--- Getting Bloomsky (STORM) data"

                data.each {oldkey, datum->
                    def key = oldkey.toLowerCase() //bloomsky returns camel cased keys. Put to lowercase so dth can update correctly
                    if (state.debug) log.debug "Storm Data: Key:Data is ${key}:${datum}"
                    ...
                    switch(key) {
						...
						case "rainrate":
                            if (datum.toDouble() < 9000) {
                                ...
                                if (("false" == rainMm)) {
                                    ...
                                    rainRate =  (rainRate * 25.4).round(1).trunc(1)
                                }
                            }
						...
						case "24hrain":
                            if (datum.toDouble() < 9000) {
                                last24hRain = datum.toDouble()
                                if (("false" == rainMm)) {
                                    last24hRain = last24hRain.trunc(1)
                                } else {
                                    last24hRain =  (last24hRain * 25.4).round(1).trunc(1)
                                }
                            }
                            sendEvent(name:"last24hRain", value: last24hRain, unit: rainUnit)
                        break;
                ...
                msgStorm = "Wind: " + windSpeed.toString() + " " + windSpeedUnit + " / " + windDirection + "\nGusts: " + windGust.toString() + " " + windSpeedUnit
                msgStormRain = "Rain Rate: " + rainRate.toString() + " " + rainUnit + "/h\nDaily: " + rainDaily.toString() + " " + rainUnit + "\nLast 24 Hours: " + last24hRain.toString() + " " + rainUnit
